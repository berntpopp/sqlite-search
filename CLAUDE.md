...
- **Config**: `playwright.config.ts` - 60s timeout, 1 worker (sequential), retries: 0 local / 2 CI
...
- **Build System**: electron-vite (not vue-cli or webpack) with Vite 7.x
...
- **Code style**: No semicolons, single quotes, 100 char print width, trailing commas ES5, LF line endings (prettier config in `package.json`)
- **TypeScript configs**: Split into 3 — `tsconfig.json` (project references root), `tsconfig.node.json` (Electron/build), `tsconfig.renderer.json` (Vue SPA)
- **Config files**: TypeScript format — `electron.vite.config.ts`, `vitest.config.ts`, `playwright.config.ts`
- **Auto-generated types**: `src/components.d.ts` and `src/auto-imports.d.ts` (generated by unplugin plugins, do not edit)
